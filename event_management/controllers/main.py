...
class BookingPage(http.Controller):
    ...
    def booking_page_submitt(self, **args):
        ...
        events_count_start = request.env['event.management'].sudo().search([('start_date', '<=', start_date_time), ('end_date','>=',start_date_time),('venue_id','=',auditorium)])
        events_count_end = request.env['event.management'].sudo().search([('start_date', '<=', end_date_time), ('end_date','>=',end_date_time),('venue_id','=',auditorium)])
